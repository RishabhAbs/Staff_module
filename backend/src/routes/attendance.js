...
router.post('/checkin', auth, async (req, res) => {
  ...
  try {
    // Fetch user's policy and shift
    const [[user]] = await db.query('SELECT checkin_policy, shift, name FROM staff WHERE id=?', [req.user.id]);

    const policy = user?.checkin_policy || 'always';

    if (policy === 'never') {
      return res.status(403).json({ message: 'Check-in is disabled for your profile.', code: 'POLICY_DENIED' });
    }

    if (policy === 'shift_based' && user?.shift) {
      const [[shift]] = await db.query('SELECT start_time, end_time FROM shifts WHERE name=?', [user.shift]);
      if (shift) {
        const now = dayjs();
        const currentMin = now.hour() * 60 + now.minute();
        const startMin = parseShiftTime(shift.start_time);
        const endMin   = parseShiftTime(shift.end_time);
        if (startMin !== null && endMin !== null) {
          ...
          if (!inShift) {
            // Notify admin
            await db.query(
              'INSERT INTO notifications (type, message, staff_id) VALUES (?, ?, ?)',
              ['shift_violation', `${user.name} attempted check-in outside shift hours (${shift.start_time} – ${shift.end_time}) at ${now.format('hh:mm A')}`, req.user.id]
            ).catch(() => {});
            return res.status(403).json({
              message: `Check-in not allowed outside your shift hours (${shift.start_time} – ${shift.end_time}).`,
    ...
    if (existing[0]) return res.status(400).json({ message: 'Already checked in today', code: 'ALREADY_CHECKED_IN' });
...
router.post('/force', auth, async (req, res) => {
  const { staffId, staff_id, date, status, check_in, check_out, type, time } = req.body;

  const finalStaffId = staffId || staff_id;
  if (!finalStaffId) return res.status(400).json({ message: 'staff_id is required' });

  const finalCheckIn = type === 'check_in' ? time : (check_in || null);
  const finalCheckOut = type === 'check_out' ? time : (check_out || null);
  ...
  try {
    await db.query(
      ...
      [finalStaffId, date, status || 'present', finalCheckIn, finalCheckOut]
    );
